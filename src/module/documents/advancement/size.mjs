...
export default class SizeAdvancement extends Advancement {
  ...
  static get metadata() {
    return foundry.utils.mergeObject(super.metadata, {
      ...
      order: 25,
      icon: "systems/anatrpg/icons/svg/size.svg",
  ...
  get automaticHint() {
    ...
    if ( this.configuration.sizes.size === 1 ) return game.i18n.format("DND5E.AdvancementSizeFlowHintSingle", {
      size: CONFIG.ANAT.actorSizes[this.configuration.sizes.first()].label
    });

    const listFormatter = new Intl.ListFormat(game.i18n.lang, { type: "disjunction" });
    return game.i18n.format("DND5E.AdvancementSizeflowHintMultiple", {
      sizes: listFormatter.format(this.configuration.sizes.map(s => CONFIG.ANAT.actorSizes[s].label))
    });
  }
  ...
  summaryForLevel(level, { configMode=false }={}) {
    ...
    return sizes.map(s => `<span class="tag">${CONFIG.ANAT.actorSizes[s].label}</span>`).join("");
  }
